...
print "Usage: perl extractintergenicordered.pl fastafile.fasta gff3file.gff3 nameofoutput\n";
...
while ( my $line = <GFF> ) {
    ...
    #Skip if changing between chromosomes.
    ...
    #If frame has been initialized, we are at least in gene #2 (meaning we can extract the first intergenic region).
    if ($last_gene_frame ne 'o') {

        $total_sequences_extracted += 1;
        my $current_frame = $array[6];
        my $gene_name = $total_sequences_extracted;
